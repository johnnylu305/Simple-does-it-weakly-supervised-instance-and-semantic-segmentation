...
class Divider:
    ...
    def load_image(self):
        ...
        for filename in tqdm.tqdm(
                os.listdir(self.img_dir_path),
                desc='{:{}}'.format('Load image names', SPACE),
    ...
    def divide(self):
        ...
        print('{:{}}: {}'.format('Training set size',
                                 SPACE, train_set.size))
    ...
    def save_set(self, set_, filename):
        with open(self.dataset_path + '/' + filename, 'w') as w:
            for name in tqdm.tqdm(
                    set_, desc='{:{}}'.format('Saving testing set', SPACE),
                    unit_scale=UNIT_SCALE, bar_format=BAR_FORMAT):
                w.write(name + '\n')
            w.close()
