...
RESTORE_CKPT_PATH = BASEDIR + "/Model/models/model_"+ str(RESTORE_TARGET) + ".ckpt"
...
def build_conv(input_, shape, name, weight_decay = WEIGHT_DECAY, strides = [1, 1, 1, 1], padding = 'SAME', activation = True, c_name = 'PRETRAIN_VGG16', holes = None):
    ...
    with tf.variable_scope(name, reuse = tf.AUTO_REUSE):
        ...
        kernel = tf.get_variable(initializer = w_initializer, shape = shape, name = 'weights', collections = [c_name, tf.GraphKeys.GLOBAL_VARIABLES], regularizer = regularizer)
        # convolution
        if not holes: 
        ...
        if activation:
            layer = tf.nn.relu(tf.layers.batch_normalization(inputs = layer, axis = -1, training = IS_TRAIN))
...
def network():
    ...
    lr = tf.train.exponential_decay(LR, global_step, DECAY_STEP, DECAY_RATE, STAIRCASE) 
    # DeepLab-LargeFOV
    with tf.variable_scope('vgg_16'): 
        ...
        layer16 = build_conv(dropout7, [1, 1, 1024, CLASS], 'fc8', padding = 'VALID', activation = False, c_name = 'UNPRETRAIN_LAST')
    ...
    prob_prediction = tf.reshape(tf.nn.softmax(predictions), [BATCH_SIZE, HEIGHT, WIDTH, CLASS])
    ...
    with tf.variable_scope('optimizer'):
        ...
        with tf.control_dependencies(update_ops):
            optimizer1 = tf.train.MomentumOptimizer(learning_rate = lr, momentum = MOMENTUM).minimize(loss, var_list = other_var)
            optimizer2 = tf.train.MomentumOptimizer(learning_rate = lr*10, momentum = MOMENTUM).minimize(loss, var_list = last_var)
    ...
    with tf.Session() as sess:
        ...
        for i in range(RESTORE_TARGET, EPOCH):
            ...
            for j in tqdm.tqdm(range(ITER), desc = '{:{}}'.format('Epoch' + str(i), SPACE), unit_scale = UNIT_SCALE, bar_format = BAR_FORMAT):
                # check empty or not
                if x_train_[j].size:
                    summary, optimizer_, loss_ = sess.run([merged, optimizer, loss], feed_dict={xp: x_train_[j], yp: y_train_[j], global_step: i})
...
def test_network(x_test, img_names):
    with tf.Session(config = config) as sess:
        ...
        for i in tqdm.tqdm(range(TEST_SIZE), desc = '{:{}}'.format('Test and save', SPACE), unit_scale = UNIT_SCALE, bar_format = BAR_FORMAT):
            predictions_, prob_predictions_ = sess.run([predictions, prob_predictions], feed_dict={xp: [x_test_[i]]})
            save_ = Save(x_test[i].astype(np.uint8), np.squeeze(predictions_), img_names[i], PRED_DIR_PATH, PAIR_DIR_PATH, CLASS)
            save_.save()

            dense_CRF_ = dense_CRF(x_test[i].astype(np.uint8), prob_predictions_[0])
            crf_mask = dense_CRF_.run_dense_CRF()
            save_ = Save(x_test[i].astype(np.uint8), crf_mask, img_names[i], CRF_DIR_PATH, CRF_PAIR_DIR_PATH, CLASS)
            save_.save()
