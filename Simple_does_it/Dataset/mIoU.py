...
class IoU:
    ...
    def count_mIoU(self):
        ...
        with open(self.dataset + '/' + self.set_name, 'r') as r:
            for i, img_name in enumerate(tqdm.tqdm(r, desc='{:{}}'.format('Load image', SPACE), unit_scale = UNIT_SCALE), start = 1):
                # get image name
                img_name = img_name.rstrip()
                # get Ground Truth
                GT = Image.open(self.dataset + '/' + self.GT_dir_name + '/' + img_name  + '.png')
                #GT = GT.resize((513, 513), Image.NEAREST)
                GT = np.array(GT)
                GT = np.where((GT == 255), 0, GT)
                height, width = GT.shape
                # get prediction
                Pred = Image.open(self.dataset + '/' + self.Pred_dir_name + '/' + img_name  + '.png')
                Pred = np.array(Pred)
            ...
            r.close()
        # count IoU
        IoU = np.divide(self.intersection, self.union, out = np.ones(self.classes), where = self.union != 0)
